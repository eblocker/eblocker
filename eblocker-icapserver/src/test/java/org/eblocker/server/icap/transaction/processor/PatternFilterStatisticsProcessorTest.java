/*
 * Copyright 2020 eBlocker Open Source UG (haftungsbeschraenkt)
 *
 * Licensed under the EUPL, Version 1.2 or - as soon they will be
 * approved by the European Commission - subsequent versions of the EUPL
 * (the "License"); You may not use this work except in compliance with
 * the License. You may obtain a copy of the License at:
 *
 *   https://joinup.ec.europa.eu/page/eupl-text-11-12
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" basis,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or
 * implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */
package org.eblocker.server.icap.transaction.processor;

import org.eblocker.server.common.data.Device;
import org.eblocker.server.common.data.FilterMode;
import org.eblocker.server.common.data.IpAddress;
import org.eblocker.server.common.service.FilterStatisticsService;
import org.eblocker.server.common.session.Session;
import org.eblocker.server.common.ssl.SslService;
import org.eblocker.server.http.service.DeviceService;
import org.eblocker.server.icap.transaction.Transaction;
import org.junit.Assert;
import org.junit.Before;
import org.junit.Test;
import org.mockito.Mockito;

public class PatternFilterStatisticsProcessorTest {

    private static final String DEVICE_ID = "device:000000000000";

    private DeviceService deviceService;
    private FilterStatisticsService filterStatisticsService;
    private PatternFilterStatisticsProcessor processor;
    private SslService sslService;


    private Device device;
    private Session session;
    private Transaction transaction;

    @Before
    public void setUp() {
        device = new Device();
        device.setId(DEVICE_ID);

        deviceService = Mockito.mock(DeviceService.class);
        Mockito.when(deviceService.getDeviceById(DEVICE_ID)).thenReturn(device);

        filterStatisticsService = Mockito.mock(FilterStatisticsService.class);
        sslService = Mockito.mock(SslService.class);

        session = Mockito.mock(Session.class);
        Mockito.when(session.getDeviceId()).thenReturn(DEVICE_ID);

        transaction = Mockito.mock(Transaction.class);
        Mockito.when(transaction.getSession()).thenReturn(session);
        Mockito.when(transaction.getOriginalClientIP()).thenReturn(IpAddress.parse("192.168.3.11"));

        processor = new PatternFilterStatisticsProcessor(false, deviceService, filterStatisticsService, sslService);
    }

    @Test
    public void testFilterModeNone() {
        device.setFilterMode(FilterMode.NONE);
        Assert.assertTrue(processor.process(transaction));
        Mockito.verify(session).setPatternFiltersEnabled(false);
        Mockito.verifyZeroInteractions(filterStatisticsService);
    }

    @Test
    public void testFilterModePlugAndPlay() {
        device.setFilterMode(FilterMode.PLUG_AND_PLAY);
        Assert.assertTrue(processor.process(transaction));
        Mockito.verify(session).setPatternFiltersEnabled(false);
        Mockito.verifyZeroInteractions(filterStatisticsService);
    }

    @Test
    public void testFilterModePlugAndPlayEnabledAdvance() {
        processor = new PatternFilterStatisticsProcessor(true, deviceService, filterStatisticsService, sslService);
        device.setFilterMode(FilterMode.PLUG_AND_PLAY);
        Assert.assertTrue(processor.process(transaction));
        Mockito.verify(session).setPatternFiltersEnabled(true);
        Mockito.verify(filterStatisticsService).countQuery("pattern", IpAddress.parse("192.168.3.11"));
    }

    @Test
    public void testFilterModeAdvanced() {
        device.setFilterMode(FilterMode.ADVANCED);
        Assert.assertTrue(processor.process(transaction));
        Mockito.verify(session).setPatternFiltersEnabled(true);
        Mockito.verify(filterStatisticsService).countQuery("pattern", IpAddress.parse("192.168.3.11"));
    }

    @Test
    public void testFilterModeAutomaticSslEnabled() {
        device.setFilterMode(FilterMode.AUTOMATIC);
        device.setSslEnabled(true);
        Mockito.when(sslService.isSslEnabled()).thenReturn(true);
        Assert.assertTrue(processor.process(transaction));
        Mockito.verify(session).setPatternFiltersEnabled(true);
        Mockito.verify(filterStatisticsService).countQuery("pattern", IpAddress.parse("192.168.3.11"));
    }

    @Test
    public void testFilterModeAutomaticSslDisabled() {
        device.setFilterMode(FilterMode.AUTOMATIC);
        device.setSslEnabled(false);
        Mockito.when(sslService.isSslEnabled()).thenReturn(false);
        Assert.assertTrue(processor.process(transaction));
        Mockito.verify(session).setPatternFiltersEnabled(false);
        Mockito.verifyZeroInteractions(filterStatisticsService);
    }

}
