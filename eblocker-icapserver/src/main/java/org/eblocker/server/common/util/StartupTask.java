...
public class StartupTask implements Runnable {
    ...
    @Inject
    public StartupTask(DeviceService deviceService, PauseDeviceController pauseDeviceController) {
    ...
    @Override
    public void run() {
        // All devices that were paused must be re-enabled
        for (Device device : deviceService.getDevices(false)) {
            if (device.isPaused()) {
                pauseDeviceController.reactivateDevice(device);
            }
        }
    }
