...
public class MobileConnectionCheckServiceTest {
    ...
    public void setUp() {
        ...
        WireGuardMobileService wireGuardMobileService = Mockito.mock(WireGuardMobileService.class);
        Mockito.when(wireGuardMobileService.getWireGuardMappedPort()).thenReturn(portNum);

        dataSource = Mockito.mock(DataSource.class);

        int portNum = 1194;

        service = new MobileConnectionCheckService(upnpService, executorService, provider, wireGuardMobileService,
    ...
    public void testStartOpensPort() throws UpnpPortForwardingException {
        Mockito.when(dataSource.getWireGuardMobilePortForwardingMode()).thenReturn(PortForwardingMode.AUTO);

        service.start();

        Mockito.verify(dataSource).getWireGuardMobilePortForwardingMode();
