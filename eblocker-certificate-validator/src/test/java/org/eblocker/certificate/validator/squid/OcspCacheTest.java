...
class OcspCacheTest {
    ...
    void testCaching() throws IOException {
        ...
        Assertions.assertEquals(response, cachedResponse);
    }
    ...
    void testRequest() throws IOException {
        ...
        Assertions.assertEquals(1, request.getRequestList().length);
        Assertions.assertEquals(CERTIFICATES[0].getSerialNumber(), request.getRequestList()[0].getCertID().getSerialNumber());
    }

    @Test
    void testHttpResponseError() throws IOException {
        HttpURLConnection connection = HttpTestUtils.createMockResponse(500);
        Mockito.when(connectionBuilder.post(Mockito.any(byte[].class))).thenReturn(connection);

        OcspCache cache = new OcspCache(4, 1, 10000, 10000, TestClock.systemUTC(), connectionBuilderFactory);
        Assertions.assertNull(cache.getOcspResponse(CERTIFICATES[0], CERTIFICATES[1]));
    }
    ...
    @Test
    void testOcspResponseParseError() throws IOException {
    ...
    @Test
    void testMaxSizeEviction() throws IOException {
        ...
        Assertions.assertNotEquals(response0, response1);
        ...
        Assertions.assertNotEquals(response0, response1);
        Assertions.assertFalse(Arrays.equals(response0, response1));
        Assertions.assertNotEquals(response1, response2);
        Assertions.assertFalse(Arrays.equals(response1, response2));
        Assertions.assertNotEquals(response0, response2);
        Assertions.assertArrayEquals(response0, response2);
    }

    @Test
    void testExpirationOnNextUpdate() throws IOException {
    ...
    @Test
    void testExpirationOnSuccessMaxAge() throws IOException {
        TestClock clock = new TestClock(LocalDateTime.of(2017, 11, 8, 0, 0, 0).toInstant(ZoneOffset.UTC));
    ...
    @Test
    void testExpirationOnErrorMaxAge() throws IOException {
        TestClock clock = new TestClock(LocalDateTime.of(2017, 11, 8, 0, 0, 0).toInstant(ZoneOffset.UTC));
    ...
    void testSerialization() throws IOException, InterruptedException {
        ...
        Assertions.assertEquals(cache.size(), deserializedCache.size());
        Assertions.assertArrayEquals(cache.getOcspResponse(CERTIFICATES[0], ISSUER_CERTIFICATES[0]), deserializedCache.getOcspResponse(CERTIFICATES[0], ISSUER_CERTIFICATES[0]));
        Assertions.assertArrayEquals(cache.getOcspResponse(CERTIFICATES[1], ISSUER_CERTIFICATES[1]), deserializedCache.getOcspResponse(CERTIFICATES[1], ISSUER_CERTIFICATES[1]));
        Mockito.verify(connectionBuilder, Mockito.times(2)).post(Mockito.any(byte[].class));
    }
}
