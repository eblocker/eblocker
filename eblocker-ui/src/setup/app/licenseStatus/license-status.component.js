/*
 * Copyright 2020 eBlocker Open Source UG (haftungsbeschraenkt)
 *
 * Licensed under the EUPL, Version 1.2 or - as soon they will be
 * approved by the European Commission - subsequent versions of the EUPL
 * (the "License"); You may not use this work except in compliance with
 * the License. You may obtain a copy of the License at:
 *
 *   https://joinup.ec.europa.eu/page/eupl-text-11-12
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" basis,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or
 * implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */
export default {
    templateUrl: 'app/licenseStatus/license-status.component.html',
    controller: LicenseStatusController
};

function LicenseStatusController($window, logger, LanguageService, LicenseService) {
    'ngInject';
    'use strict';

    var vm = this;
    vm.getLicenseStatus = getLicenseStatus;
    vm.openConsole = openConsole;

    function openConsole() {
        $window.open('/', 'eblocker.console');
    }

    function getLicenseStatus() {
        LicenseService.getLicenseStatus().then(function success(response) {
            vm.license = response.data;
            if (vm.license.productInfo && vm.license.productInfo.productName) {
                vm.license.displayProductName = vm.license.productInfo.productName.replace(/\s*\(.*\)\s*$/, '');
            }
            vm.license.notValidAfterDisplay = LanguageService.getDate(vm.license.notValidAfter,
                    'LICENSE_STATUS.FORMAT_DATE');
        });
    }
    vm.getLicenseStatus();
}
