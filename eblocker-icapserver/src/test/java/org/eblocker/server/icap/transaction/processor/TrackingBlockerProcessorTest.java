...
public class TrackingBlockerProcessorTest {

    private Filter filter;
    private Transaction transaction;
    private FilterResult transactionFilterResult;
    ...
    public void setUp() {
        ...
        Mockito.when(transaction.getOriginalClientIP()).thenReturn(Ip4Address.parse("192.168.3.3"));
        ...
        Mockito.when(transaction.getFilterResult()).then(im -> transactionFilterResult);
        Mockito.doAnswer(im -> transactionFilterResult = im.getArgument(0)).when(transaction).setFilterResult(Mockito.any());
    ...
    public void testFilterBlock() {
        ...
        Mockito.verify(patternBlockerUtils).countBlockedDomain(Mockito.eq(Category.TRACKERS), Mockito.any(FilterResult.class), Mockito.eq(session), Mockito.eq(transaction));
    }
