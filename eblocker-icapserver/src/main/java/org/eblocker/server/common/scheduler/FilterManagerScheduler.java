/*
 * Copyright 2020 eBlocker Open Source UG (haftungsbeschraenkt)
 *
 * Licensed under the EUPL, Version 1.2 or - as soon they will be
 * approved by the European Commission - subsequent versions of the EUPL
 * (the "License"); You may not use this work except in compliance with
 * the License. You may obtain a copy of the License at:
 *
 *   https://joinup.ec.europa.eu/page/eupl-text-11-12
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" basis,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or
 * implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */
package org.eblocker.server.common.scheduler;

import com.google.inject.Inject;
import com.google.inject.name.Named;
import org.eblocker.server.icap.filter.FilterManager;

import java.util.concurrent.ScheduledExecutorService;
import java.util.concurrent.TimeUnit;

public class FilterManagerScheduler implements Scheduler {
    private final FilterManager filterManager;
    private final long asynchronousLearnerStartupDelay;
    private final long asynchronousLearnerFixedDelay;
    private final long filterStoreUpdaterStartupDelay;
    private final long filterStoreUpdaterFixedDelay;

    @Inject
    public FilterManagerScheduler(
            FilterManager filterManager,
            @Named("executor.asynchronousLearner.startupDelay") long asynchronousLearnerStartupDelay,
            @Named("executor.asynchronousLearner.fixedDelay") long asynchronousLearnerFixedDelay,
            @Named("executor.filterStoreUpdater.startupDelay") long filterStoreUpdaterStartupDelay,
            @Named("executor.filterStoreUpdater.fixedDelay") long filterStoreUpdaterFixedDelay
    ) {
        this.filterManager = filterManager;
        this.filterStoreUpdaterStartupDelay = filterStoreUpdaterStartupDelay;
        this.filterStoreUpdaterFixedDelay = filterStoreUpdaterFixedDelay;
        this.asynchronousLearnerStartupDelay = asynchronousLearnerStartupDelay;
        this.asynchronousLearnerFixedDelay = asynchronousLearnerFixedDelay;
    }

    @Override
    public void schedule(ScheduledExecutorService service) {
        service.scheduleAtFixedRate(filterManager.getFilterStoreUpdater(), filterStoreUpdaterStartupDelay, filterStoreUpdaterFixedDelay, TimeUnit.SECONDS);
        service.scheduleAtFixedRate(filterManager.getAsynchronousLearnersUpdater(), asynchronousLearnerStartupDelay, asynchronousLearnerFixedDelay, TimeUnit.SECONDS);
    }

}
