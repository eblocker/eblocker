 ...
 */
export default {
    templateUrl: 'app/components/parentalControl/whitelists/whitelists.component.html',
...
function Controller(logger, $translate, $interval, BlockerService, LanguageService, StateService, UserProfileService,
    ...
    // ** START: TABLE
    vm.tableId = TableService.getUniqueTableId('parentalcontrol-whitelist-table');
    vm.detailsState = 'whitelistdetails';
    vm.tableHeaderConfig = [
        {
            label: '',
            icon: '/img/icons/ic_check_black.svg',
            isSortable: true,
            isXsColumn: true,
            sortingKey: 'assignedToProfile'
        },
        {
            label: 'ADMINCONSOLE.WHITELISTS.TABLE.COLUMN.NAME',
            isSortable: true,
            flexGtXs: 25,
            sortingKey: 'localizedName'
        },
        {
            label: 'ADMINCONSOLE.WHITELISTS.TABLE.COLUMN.DESCRIPTION',
            isSortable: true,
            sortingKey: 'localizedDescription'
        },
        {
            label: 'ADMINCONSOLE.WHITELISTS.TABLE.COLUMN.LAST_MODIFIED',
            isSortable: true,
            flex: 15,
    ...
    function loadAllFilterLists() {
        ...
        BlockerService.getBlockers(true, config).then(function(response) {
            const filterLists = response.data.filter(function(el) {
                return el.category === 'PARENTAL_CONTROL' && el.filterType === 'whitelist';
            });
            vm.tableData = [];
            vm.filteredTableData = [];
            filterlistMap = {};
            const dateFormat = 'ADMINCONSOLE.WHITELISTS.DATE_FORMAT';
    ...
    function newWhitelist(event){
        newFilterList('whitelist', event);
    }
    ...
    function addNewFilterListDialog(module, event){
        DialogService.filterNewEdit(module, event, 'whitelist').then(loadAllFilterLists, function() {});
    }
}
